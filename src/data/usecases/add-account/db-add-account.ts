...
export class DbAddAccount implements AddAccount {
  constructor (
    ...
    private readonly loadAccountByEmailRepositury: LoadAccountByEmailRepository
  ) {}

  async add (accountData: AddAccountModel): Promise<AccountModel> {
    const account = await this.loadAccountByEmailRepositury.loadByEmail(accountData.email)
    if (!account) {
