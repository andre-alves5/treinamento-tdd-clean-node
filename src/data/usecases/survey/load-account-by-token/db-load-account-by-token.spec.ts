...
describe('DbLoadAccountByToken Usecase', () => {
  ...
  test('Should retutn null if Decrypter returns null', async () => {
    const { sut, decrypterSpy } = makeSut()
  ...
  test('Should retutn null if LoadAccountByTokenRepository returns null', async () => {
    const { sut, loadAccountByTokenRepositorySpy } = makeSut()
  ...
  test('Should retutn an account on success', async () => {
    const { sut, loadAccountByTokenRepositorySpy } = makeSut()
    const account = await sut.load(token, role)
    expect(account).toBe(loadAccountByTokenRepositorySpy.accountModel)
  })
